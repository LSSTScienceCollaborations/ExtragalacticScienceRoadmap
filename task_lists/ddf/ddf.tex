...
~\\
\activities{
The major challenge in supporting the LSST DDFs is the huge investment of telescope time.
Providing supporting multiwavelength data sets requires
coordination across facilities and collaborations to make the most efficient use of telescope time. Current coordination occurs somewhat haphazardly, but there has not to date been a dedicated effort to organize potential stakeholders involved in developing a coherent plan. The LSST Science Collaborations can and should take the lead here.
...
further work remains. Activities include: 
\begin{itemize}
...
\item Execution of supporting programs.
\item Work to integrate the data from supporting programs with the LSST data.
\item Work to enable DDF support through policies and strategic planning at major observatories.
\end{itemize}
}
~\\
...
But justifying this investment will require work, including the selection of a suitable DDF
...
\motivation{
Getting the most out of the DDFs may require data processing beyond that required for the WFD Survey. A variety of issues will need consideration in trying to optimize the science output,
including different strategies for making co-adds, determining sky levels, treating scattered light, detecting and characterizing faint or low-surface brightness features, deblending overlapping objects, or estimating photometric redshifts. Reprocessing the DDF data while utilizing
...
\\
Develop the machinery to test and validate the data-processing on the DDFs (via pure simulations and artificial-source injection). This activity may stress the inputs to the image simulator, requiring more realistic inputs for low-mass galaxies, galaxy morphologies, and low-surface brightness features. Use of the supporting data sets in Level 2 or 3 processing requires careful thought. For example, 
...
\item Level 3 data processing code, or augmentations to the LSST Level 2 pipeline.
\end{enumerate}
}
\end{task}
